...
import styles from "./inputsMatriceria.module.css"

const InputMatriceria = ()=>{
  const [inputs, setInputs] = useState({});
  const[showModal,setShowModal]=useState(false)
  const[errorMsg, SetErrorMsg] =useState('')
  const [arrErrors, setArrErrors]= useState([])
  const [piezas, setPiezas] = useState([])
  const depoUser = sessionStorage.getItem('InyeccionUser')
  console.log ('Inputs: ',inputs)
  useEffect(()=>{
    fetchingPiezas()
  },[])
  const fetchingPiezas = ()=>{
    fetch('http://192.168.11.139'+ process.env.REACT_APP_PORTS +'/api/matriceria/moldes')
      .then((res)=>res.json())
      .then ((json)=>{
        setPiezas(json)
      })
  ...
  const handleSubmit = (event) => {
    ...
    fetch('http://192.168.11.139'+ process.env.REACT_APP_PORTS +'/api/matriceria/upload',options)
      .then((res)=>{
...
export default InputMatriceria
